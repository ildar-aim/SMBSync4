...
### 1.1.Données fournies par l'utilisateur à SMBSync4  
...
- Mot de passe pour protéger les données dans "1.4 Envoi ou écriture de données en dehors de SMBSync4"  
Le mot de passe sera éliminé et non sauvegardé lorsque le processus sera terminé.  

### 1.2.Résultat de l'exécution de SMBSync4  

Sauvegarder les données dans la zone de stockage de l'application afin que l'utilisateur puisse vérifier le résultat de l'exécution de SMBSync4.  
...
### 1.3.Journal d'activité de SMBSync4  
...
- Version de SMBSync4, options d'exécution de SMBSync4  
- Nom du répertoire, nom du fichier, état d'exécution  
- Taille des fichiers synchronisés, date et heure de mise à jour des fichiers.  
- Informations sur le débogage  
- Informations sur les erreurs  

### 1.4 Envoi ou écriture de données en dehors de SMBSync4  

<span style="color: red;"><u>Les données de SMBSync4 ne peuvent être envoyées ou écrites vers l'extérieur que si l'utilisateur l'actionne.</u></span>  
...
- En exécutant "Exporter la configuration" à partir du menu, "1.1.Données fournies par l'utilisateur à SMBSync4" sera exporté.  
...
### 1.5.supprimer les données stockées dans SMBSync4  

En désinstallant SMBSync4, les données enregistrées ("1.1.Données fournies par l'utilisateur à SMBSync4", "1.2.Résultat de l'exécution de SMBSync4", "1.3.Journal d'activité de SMBSync4") seront supprimées de l'appareil.  
